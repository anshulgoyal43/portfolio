...
export function ThemeToggle() {
  const [preference, setPreference] = useState<undefined | string>(undefined);
  ...
  useEffect(() => {
    setPreference(localStorage.getItem("theme") || "light");
    const current = themeEffect();
    setCurrentTheme(current);
  ...
  const onStorageChange = useCallback(
    (event: StorageEvent) => {
      if (event.key === "theme") setPreference(event.newValue || "light");
    },
    [setPreference]
  );
